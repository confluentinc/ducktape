...
class CheckIterWrapper(object):
    ...
    def check_iter_wrapper_timeout(self):
        ...
        stop = time.time()
        assert (stop - start >= 5) and (stop - start) < 6, "has_next() should return right after 5 seconds"

    def teardown(self):
        self.account.ssh_capture("rm -f " + self.temp_file)
