...
class VagrantCluster(JsonCluster):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs.get("use_cluster_info", False):
            try:
                cluster_json = json.load(open(ConsoleConfig.CLUSTER_INFO_FILE))
                super(VagrantCluster, self).__init__(cluster_json)
                return
            except:
                raise RuntimeError("Cannot load cluster info from json file " + ConsoleConfig.CLUSTER_INFO_FILE)
        ...
        # cache cluster info by writing info to a local file in JSON format.
        cluster_json["is_aws"] = is_aws
