...
def main():
    ...
    def make_dict(obj):
        ...
        d = {k:v for k, v in obj.__dict__.items() if not callable(v)}
        for k, v in d.items():
            if hasattr(v, '__dict__'):
                d[k] = make_dict(v)
            elif isinstance(v, list) or isinstance(v, set):
                d[k] = [make_dict(i) for i in v if not callable(v)]
        return d
    ...
    if args_dict["collect_only"]:
        ...
        tst = json.dumps({test.module : test.cluster_use_metadata.num_nodes for test in tests})
